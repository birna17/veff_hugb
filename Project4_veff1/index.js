...
function myAuthorizer (user, pw) {
    if (sha256(user) === "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918" && sha256(pw) === "2bb80d537b1da3e38bd30361aa855686bde0eacd7162fef6a25fe97bf527a25b") {
        return true;
    }
    return false;
};
...
app.delete(apiPath + version + '/events/:eventId', (req, res) => { //add auth
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({ "error": "Event not found!" });
    }
...
app.delete(apiPath + version + '/events/:eventId/bookings/:bookingId', (req, res) => { // add auth
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({"message": "Event not found!"});
    }
